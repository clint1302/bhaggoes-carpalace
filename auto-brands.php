...
<div class="section-content col-lg-9 col-md-9 col-sm-12 col-xs-12 item-preowned-block">
		...
		if (isset($_GET['brand-id'])) {
			...
			//Get Model
			//$f_model = mysqli_fetch_assoc($con->query("SELECT car_models.title, cars.title, cars.year FROM car_models RIGHT JOIN cars ON car_models.model_id = cars.model_id  WHERE `brand_id` = '".$id."'"));
			
			//count
			$count_brands = mysqli_num_rows($con->query("SELECT car_models.*, cars.* FROM car_models RIGHT JOIN cars ON car_models.model_id = cars.model_id  WHERE `brand_id` = '".$id."' AND `status` > 0 "));
			 //echo $count_brands;
		...
		<div class="row cars-main-section">
			<div found class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="auto-sort-filter">
			<div class="auto-show-resuilt"><span>SHOWING <em><?php echo $count_brands ?></em> RESULTS</span></div>
			</div></div>
					<?php
						...
						$result = mysqli_query($con, "SELECT car_models.*, cars.* FROM car_models RIGHT JOIN cars ON car_models.model_id = cars.model_id  WHERE `brand_id` = '".$id."' AND `status` > 0 LIMIT $start, $perpage");
...
?>
	                ...
	                <nav>
		                <ul class="pagination">
											...
											<?php
														...
														$result = mysqli_query($con,"SELECT COUNT(*) AS Total FROM car_models RIGHT JOIN cars ON car_models.model_id = cars.model_id  WHERE `brand_id` = '".$id."' AND `status` > 0 ");
														...
														echo "<li><a href='auto-brands.php?page=$j&brand-id=$id' aria-label='Previous'><span aria-hidden='true'><i class='icon-angle-left'></i></span></a></li>";
														...
														echo "<li><a href='auto-brands.php?brand-id=$id&page=$i'>$i</a></li>";
														...
														echo "<li><a href='auto-brands.php?page=$j&brand-id=$id' aria-label='Next'><span aria-hidden='true'><i class='icon-angle-right'></i></span></a></li>";
		...
		<div class="brands-nothing-block">
				<div class="bnothing-title">No cars found</div>
				<a href="auto-grid.php" class="bnothing-link">Check our Inventory for more cars.</a>
				<a href="auto-grid.php"><img src="assets/car-not-found.png" alt=""></a>
		</div>
