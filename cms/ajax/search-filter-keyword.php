...
//if(isset($_COOKIE['cmsadmin'])){
	...
	//$g_filtered_cars = mysqli_query($con,"SELECT * FROM `cars` WHERE `category_id` = '.$bodystyle.' LIMIT 0,10");
	$g_filtered_cars = mysqli_query($con,"SELECT * FROM `cars` WHERE `title` LIKE '%".$keyword."%' AND `status` > 0 LIMIT 0,9");
	...
	if($count_results != 0){
		...
		while($filtered_car = mysqli_fetch_array($g_filtered_cars)){
			...
			<div class="cs-media">
				...
				<img src="'.BASEHREF.'cms/upload/cars/medium/'.$filtered_car['img'].'" alt="#"/>'.$car_status.'</a></figure>
			</div>
			<div class="auto-text">
